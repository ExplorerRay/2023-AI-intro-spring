...
class ExactInference(object):
    ...
    def observe(self, agentX: int, agentY: int, observedDist: float) -> None:
        ...
        for row in range(self.belief.getNumRows()):
            for col in range(self.belief.getNumCols()):
                ...
                cx = util.colToX(col)
                cy = util.rowToY(row)
                dist = ((agentX - cx)**2 + (agentY - cy)**2)**0.5
                likelyhood = util.pdf(dist, Const.SONAR_STD, observedDist)
                self.belief.setProb(row, col, likelyhood * prior)
    ...
    def elapseTime(self) -> None:
        ...
        next_belief = util.Belief(self.belief.getNumRows(), self.belief.getNumCols(), 0)
        for nrow in range(self.belief.getNumRows()):
            for ncol in range(self.belief.getNumCols()):
                for row in range(self.belief.getNumRows()):
                    for col in range(self.belief.getNumCols()):
                        if ((row, col),(nrow, ncol)) in self.transProb:
                            p = self.belief.getProb(row, col) * self.transProb[((row, col),(nrow, ncol))]
                            next_belief.addProb(nrow, ncol, p)
...
class ParticleFilter(object):
    ...
    def observe(self, agentX: int, agentY: int, observedDist: float) -> None:
        ...
        weightDict = collections.defaultdict(float)
        for row, col in self.particles:
            prior = self.particles[(row, col)]
            cx = util.colToX(col)
            cy = util.rowToY(row)
            dist = ((agentX - cx)**2 + (agentY - cy)**2)**0.5
            likelyhood = util.pdf(dist, Const.SONAR_STD, observedDist)
            weightDict[(row, col)] = likelyhood * prior
        
        new_particles = collections.defaultdict(int)
        for _ in range(self.NUM_PARTICLES):
            new_particles[util.weightedRandomChoice(weightDict)] += 1
